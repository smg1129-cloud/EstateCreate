...
export default async function IntakePage({ params }: { params: { kind: string } }) {
  ...
  async function submit(answers: Answers) {
    'use server'
    await assertCanAccessMatter({ id: actorId, role: 'CLIENT', organizationId: matter!.organizationId }, matterId)
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mt-6">
        <QuestionnaireForm
          ...
          action={submit}
          submitLabel={kind === 'TRIAGE' ? 'Continue to estate questionnaire' : 'Submit for preparation & review'}
        />
      </div>
    </div>
  )
