...
import { idleTimeoutMinutesFor, roleHomeFor } from '@/lib/rbac'
...
// Attorneys and paralegals share the staff review workspace; admins may also
// reach it. Admin tooling (users, audit) is admin-only.
const ROLE_PREFIXES: Record<string, Role[]> = {
  '/portal': ['CLIENT'],
  '/attorney': ['ATTORNEY', 'PARALEGAL', 'ADMIN'],
  '/admin': ['ADMIN'],
}
...
export async function middleware(req: NextRequest) {
  ...
  // than relying solely on the JWT's overall maxAge, since staff roles need a
  // much shorter idle window than the session's hard cap.
  ...
  // token's original absolute expiry and pass that, otherwise re-signing the
  // cookie on every request would silently override the 12-hour hard cap
  // configured in src/lib/auth.ts with encode()'s 30-day default.
...
export const config = {
  matcher: ['/portal/:path*', '/attorney/:path*', '/admin/:path*', '/mfa/setup'],
}
